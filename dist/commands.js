 ...
 */
export const generateHtmlToCriticalCss = async (html, outputDir) => {
    let htmlWithInlineCSS = html;
    const cssLinkRegex = /<link[^>]*rel=["']stylesheet["'][^>]*href=["']([^"']+)["'][^>]*>/g;
    let match;
    let existContent = undefined;
    const cssFiles = [];
 ...
 */
export const getCriticalHTML = async (page) => {
 ...
 */
export const extractCriticalCss = async (fictionalHtml, cssIds) => {
    // Get all CSS
    let cssGlobal = '';
    cssIds.forEach((id) => {
